...
SELECT
   Subquery.DrugCode, SUM(HoursUsed) AS HoursUsed,
   ROUND(MIN(CAST(json_unquote(json_extract(Subquery.Volume, '$[0]')) AS DOUBLE)), 3) AS DoseMin,
   ROUND(MAX(CAST(json_unquote(json_extract(Subquery.Volume, '$[0]')) AS DOUBLE)), 3) AS DoseMax
FROM (
   SELECT
    	CI.DrugCode, CI.StartDate, CI.EndDate,
      	TIMESTAMPDIFF(second, CI.StartDate, CI.EndDate) / 3600 AS HoursUsed,
      	CASE
