...
SELECT
    ...
    COALESCE((COUNT(CASE WHEN H1.Death = 0 AND
 		 H1.End IS NOT NULL AND
 		 H1.End >= @StartDate AND
 		 H1.END <= @EndDate THEN 1 END) * 100.0) / COUNT(CASE WHEN H1.End IS NOT NULL AND
                                                 		 H1.End >= @StartDate AND
                                                 		 H1.END <= @EndDate THEN 1 END), 0) AS Percent_Alta,
    ...
    COALESCE((COUNT(CASE WHEN H1.Death = 1 AND
 		 ...
 		 H1.END <= @EndDate THEN 1 END) * 100.0) / COUNT(CASE WHEN H1.End IS NOT NULL AND
                                                 		 H1.End >= @StartDate AND
                                                 		 H1.END <= @EndDate THEN 1 END), 0) AS Percent_Obito
FROM (
